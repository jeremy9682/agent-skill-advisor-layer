...
| `from_seat` | Producing seat, e.g. `claude-direction`, `claude-landing`, `codex-landing`, `codex-final-review`, `cursor-grok-cross-review`, `kimi-landing`, `glm-flash-landing`, `devin-plan`, `human`. A `cursor-*` seat records a Cursor-provider session; it is never an alias for a Codex seat. `kimi-*`, `glm-*` and `devin-*` seats are real external executor seats (Kimi / GLM / Devin provider sessions), never aliases for a Claude seat. |
