...
export function parseCursorWireModel(modelId) {
    const open = modelId.lastIndexOf('[');
    if (open < 1 || !modelId.endsWith(']'))
        return { base: modelId, parameters: {} };
...
function preferredLevel(parameters) {
    return parameters.effort ?? parameters.reasoning;
}
function baseStableId(model) {
...
export function normalizeCursorCatalog(provider, wireModels) {
    const uniqueWire = new Map();
    for (const model of wireModels) {
        if (model.modelId.trim().length === 0 || model.name.trim().length === 0)
            continue;
        if (!uniqueWire.has(model.modelId))
            uniqueWire.set(model.modelId, model);
    }
...
export function requireCatalogModel(catalog, modelId) {
    const model = catalog.find(candidate => candidate.id === modelId);
    if (model === undefined) {
        const error = new Error(`unknown Cursor ACP model "${modelId}"`);
