...
import { cursorSeatbeltProfile, isCursorLaunchVerified, resolveCursorLaunch, } from './cursor-runtime.js';
function isRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
function parseModels(session) {
    if (!isRecord(session))
        return [];
    const rows = Array.isArray(session.models) ? session.models : [];
    const models = [];
    for (const row of rows) {
        if (!isRecord(row) || typeof row.modelId !== 'string' || typeof row.name !== 'string')
            continue;
        models.push({
            modelId: row.modelId,
            name: row.name,
...
async function isolatedHome(root, hostHome) {
    const home = join(root, 'home');
    await mkdir(join(home, 'Library'), { recursive: true, mode: 0o700 });
    if (process.platform === 'darwin')
        await symlink(join(hostHome, 'Library/Keychains'), join(home, 'Library/Keychains'));
    return home;
}
/** Discover the live Cursor model catalog without exposing account fields. */
export async function probeCursorCatalog(command, headers, signal, hostHome = homedir(), maxLineBytes = 4 * 1024 * 1024) {
    ...
    const connection = new acp.ClientSideConnection(() => client, strictAcpNdJsonStream(child.stdin, child.stdout, maxLineBytes));
    ...
    try {
        ...
        if (models.length === 0)
            throw new Error('CURSOR_CATALOG_EMPTY: Cursor ACP returned no models');
        return { authenticated: true, models };
    }
    catch (error) {
        if (probeTimedOut)
            throw new Error('CURSOR_TIMEOUT: Cursor ACP catalog probe timed out');
        const evidence = `${error instanceof Error ? error.message : String(error)}\n${stderr}`.toLowerCase();
        if (/auth|login|sign[ -]?in|credential/u.test(evidence))
        ...
        if (sessionId !== undefined && closeSessionSupported)
...
//# sourceMappingURL=cursor-probe.js.map