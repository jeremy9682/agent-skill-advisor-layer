...
export function escapeCursorControlText(text) {
    return text.replaceAll(CURSOR_CONTROL_PREFIX, '␞dsh-cursor:');
}
export function encodeCursorControl(event) {
...
export function parseCursorControl(message) {
    if (!message.startsWith(CURSOR_CONTROL_PREFIX))
        return undefined;
    const value = JSON.parse(message.slice(CURSOR_CONTROL_PREFIX.length));
    if (!isRecord(value))
        throw new Error('invalid Cursor control event');
    ...
    if (isRecord(value.usage)) {
        ...
        if (inputTokens === undefined || outputTokens === undefined)
            throw new Error('invalid Cursor usage control');
        return {
            usage: {
                inputTokens,
                outputTokens,
...
//# sourceMappingURL=control-event.js.map