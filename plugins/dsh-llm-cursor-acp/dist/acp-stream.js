...
export function strictAcpNdJsonStream(output, input, maxLineBytes) {
    ...
    const drain = () => {
        if (controller === undefined || failed)
            return;
    ...
    const writable = new WritableStream({
        write(message) {
            ...
            if (Buffer.byteLength(line) > maxLineBytes)
                throw new Error(`Cursor ACP request exceeds ${String(maxLineBytes)} bytes`);
