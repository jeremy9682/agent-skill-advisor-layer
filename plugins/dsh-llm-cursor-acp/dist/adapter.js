...
import { CallId, LlmAdapter, LlmError, isAgentLoopRequest, resolveRetryPolicy, } from '@deepseek-ai/dsh-llm';
...
import { CURSOR_ACP_PROVIDER, MODEL_PROVIDER_PROTOCOL, } from './types.js';
...
function renderBlock(block) {
    switch (block.type) {
        ...
        default: return `[unsupported content block ${String(block.type)}]`;
    }
}
...
export function renderCursorTask(options, toolsAllowed) {
    const sections = [
...
function bridgeKey(options) {
    if (options.sessionId === undefined || !isAgentLoopRequest(options))
        return undefined;
...
function runtimeLlmError(message) {
...
export class CursorAcpAdapter extends LlmAdapter {
    ...
    live = new Set();
    disposing = false;
    constructor(ctx, config, refresh) {
        super();
        this.ctx = ctx;
        this.config = config;
    ...
    async *stream(options) {
        if (this.disposing)
            throw new LlmError('Cursor ACP adapter is disposing', 'TRANSPORT');
        if (options.provider !== CURSOR_ACP_PROVIDER)
            throw new LlmError(`unexpected provider ${JSON.stringify(options.provider)}`, 'PROVIDER');
        ...
        if (bridge !== undefined) {
            ...
            if (bridge.pendingTool === undefined)
                throw new LlmError('Cursor ACP bridge is already active', 'PROVIDER');
        ...
        try {
            while (true) {
                ...
                if (output.type === 'model.event') {
                    if (output.event.type === 'message.delta' && output.event.message !== undefined) {
                        ...
                        if (control !== undefined) {
                            if ('stopReason' in control)
                                terminalStop = 'max-tokens';
                            else {
                                for (const chunk of closeOpen())
                                    yield chunk;
                            ...
                            for (const chunk of delta('text', output.event.message))
                                yield chunk;
                        }
                    }
                    else if (output.event.type === 'reasoning.delta' && output.event.message !== undefined) {
                        for (const chunk of delta('reasoning', output.event.message))
                            yield chunk;
                    }
                    continue;
                }
                if (output.type === 'tool.call') {
                    for (const chunk of closeOpen())
                        yield chunk;
                    if (!agentLoop || key === undefined)
                        throw new LlmError('Cursor tool calls require a DSH Agent Loop request', 'POLICY_DENIED');
                    ...
                    if (argumentsJson === undefined)
                        throw new LlmError('Cursor returned non-JSON tool arguments', 'TRANSPORT');
                ...
                if (output.type === 'model.error')
                    throw runtimeLlmError(output.error);
                if (output.type === 'model.result') {
                    ...
                    if (open === undefined && output.result.output.length > 0) {
                        ...
                        if (residual.length > 0) {
                            ...
                            for (const chunk of delta('text', residual))
                                yield chunk;
                        }
                    }
                    for (const chunk of closeOpen())
    ...
    async reset(reason = 'Cursor ACP configuration changed') {
    ...
    resolveConfiguredModel(modelId, config = this.config()) {
        const model = [...config.models, ...config.tombstones].find(candidate => candidate.id === modelId);
        if (model === undefined)
            throw new LlmError(`unknown Cursor ACP model ${JSON.stringify(modelId)}`, 'UNKNOWN_MODEL');
        return model;
    }
    async startBridge(options, model, key, config) {
        ...
        void process.waitForExit().then(async () => {
            ...
            if (this.active.get(bridge.key) === bridge)
                this.active.delete(bridge.key);
    ...
    assertOutputOwner(bridge, output) {
    ...
    async closeBridge(bridge, reason, graceful = true) {
        if (bridge.cancelling !== undefined)
            return bridge.cancelling;
        if (bridge.disposed)
            return;
        const closing = (async () => {
            bridge.disposed = true;
            ...
            if (!bridge.abort.signal.aborted)
                bridge.abort.abort(new Error(reason));
