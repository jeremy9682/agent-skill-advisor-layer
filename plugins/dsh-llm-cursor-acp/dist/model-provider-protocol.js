...
import { MODEL_PROVIDER_PROTOCOL } from './types.js';
function isRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
/** Parse and validate one untrusted Agent Virtualization output line. */
export function parseModelProviderOutput(line) {
...
export class ModelProviderMailbox {
    ...
    waiters = [];
    closed;
    constructor(input, maxLineBytes) {
        this.maxLineBytes = maxLineBytes;
        if (!Number.isSafeInteger(maxLineBytes) || maxLineBytes <= 0)
            throw new Error('maxLineBytes must be positive');
...
export function writeModelProviderInput(output, message) {
    return new Promise((resolve, reject) => {
        output.write(`${JSON.stringify(message)}\n`, error => error === null || error === undefined ? resolve() : reject(error));
    });
}
//# sourceMappingURL=model-provider-protocol.js.map