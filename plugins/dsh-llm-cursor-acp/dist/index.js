...
import { CURSOR_ACP_PROVIDER, DEFAULT_CURSOR_MODEL, } from './types.js';
...
export function resolveConfig(config) {
    ...
    if (command === undefined)
        throw new Error('llm-cursor-acp: cursor-agent is not installed or command is not configured');
    const values = {
        command,
    ...
    for (const [key, value] of Object.entries(values)) {
        if (key === 'command' || key === 'defaultModel')
            continue;
        if (!Number.isSafeInteger(value) || Number(value) <= 0)
            throw new Error(`llm-cursor-acp: ${key} must be a positive safe integer`);
    }
    return values;
}
...
export function providerCacheRoot() {
    return join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'cache', 'llm-cursor-acp');
}
export function createCatalogStore(config) {
...
export async function apply(ctx, config) {
    ...
    const operationSignal = (signal) => signal === undefined
        ? lifecycle.signal
    ...
    installSettingsSection(ctx, SETTINGS_NAMESPACE, Config, config, {
        ...
        onChange: () => {
            ...
            if (!ready || currentAdapter === undefined)
                return;
            reconfigure = reconfigure.then(async () => {
                const next = options();
                if (configFingerprint(next) === configFingerprint(activeConfig))
                    return;
    ...
    lifecycle.signal.throwIfAborted();
    const registeredAdapter = new CursorAcpAdapter(ctx, () => store.adapterConfig(), (force, signal, requireSuccess) => store.refresh(force, operationSignal(signal), requireSuccess));
    ...
    ctx.on('session/disposed', (session) => { void registeredAdapter.disposeSession(String(session.id)); });
    ctx.on('session/event', (session, event) => {
        if (event.type === 'turn/end')
            void registeredAdapter.disposeSession(String(session.id));
    });
        ...
        const handler = async (endpoint, payload, signal) => {
            try {
                if (endpoint === 'status' || endpoint === 'refresh') {
                    if (endpoint === 'refresh')
                        await store.refresh(true, operationSignal(signal), true);
                ...
                return { ok: false, error: { code: 'internal', message: 'unknown Cursor ACP endpoint', details: {} } };
            }
            catch (error) {
