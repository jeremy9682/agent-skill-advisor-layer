...
import { runCursorGenericRuntime } from './cursor-runtime.js';
...
export function parseRuntimeArguments(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (key === undefined || !key.startsWith('--') || value === undefined)
            throw new Error(`invalid runtime argument ${String(key)}`);
        const existing = values.get(key) ?? [];
        existing.push(value);
        values.set(key, existing);
        index += 1;
    }
    const one = (name) => values.get(name)?.at(-1);
    const cursorCommand = one('--cursor-command');
    const wireModel = one('--wire-model');
    if (cursorCommand === undefined || wireModel === undefined)
        throw new Error('--cursor-command and --wire-model are required');
    const headers = {};
    for (const header of values.get('--header') ?? []) {
        const separator = header.indexOf(':');
        if (separator <= 0)
            throw new Error('runtime --header must use name:value');
...
async function main() {
    ...
    do {
        ...
        if (first.done)
            throw new Error('generic runtime input closed before run.start');
...
//# sourceMappingURL=runtime-bin.js.map