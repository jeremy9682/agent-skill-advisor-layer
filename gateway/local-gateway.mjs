 ...
 *
 * Surfaces:
 *   dsh          `dsh --profile ${DSH_GATEWAY_PROFILE:-headless}`
...
export const DEFAULT_GATEWAY_PROFILE = "headless";
export const CURSOR_ACP_ADAPTER = "@jeremy9682/dsh-llm-cursor-acp";
...
Env: DSH_GATEWAY_PROFILE (default headless). MCP/gateway --via dsh never auto-switch to web.
...
export function resolveGatewayProfile(env = process.env) {
  const raw = env.DSH_GATEWAY_PROFILE;
...
function readDefaultProvider(dshHome) {
  ...
  const block = text.match(/^agent-default-model:\n((?:[ \t].*\n)*)/m);
  if (!block) return null;
  const provider = block[1].match(/^[ \t]+provider:[ \t]*(\S+)/m);
  return provider ? provider[1] : null;
}

function profileHasPackage(dshHome, profile, pkg) {
  const manifest = join(dshHome, "profiles", profile, "package.json");
  if (!existsSync(manifest)) return false;
  try {
    const json = JSON.parse(readFileSync(manifest, "utf8"));
    if (json.dependencies && Object.prototype.hasOwnProperty.call(json.dependencies, pkg)) {
      return true;
    }
    const bundles = json.dsh?.profile?.bundles;
    return Array.isArray(bundles) && bundles.includes(pkg);
  } catch {
    return false;
  }
}

export function missingCursorAcpAdapterWarning(dshHome, profile) {
  const provider = readDefaultProvider(dshHome);
  if (provider !== "cursor-acp") return null;
  if (profileHasPackage(dshHome, profile, CURSOR_ACP_ADAPTER)) return null;
  return (
    `default provider is cursor-acp but profile "${profile}" has no ${CURSOR_ACP_ADAPTER} adapter; ` +
    `MCP and gateway --via dsh will fail with NO_ADAPTER. ` +
    `Install with the same source as web: dsh plugin --profile ${profile} add <tgz-or-path>. ` +
    `Do not switch the gateway to web (UI). Override only via DSH_GATEWAY_PROFILE.`
  );
}
...
export function doctor(env = process.env, home = homedir()) {
  ...
  const profile = resolveGatewayProfile(env);
  const dshHome = resolveDshHome(env, home);
  const adapterWarning = missingCursorAcpAdapterWarning(dshHome, profile);
  return {
    ok: surfacesOk,
    name: NAME,
    version: VERSION,
    profile,
    warnings: adapterWarning ? [adapterWarning] : [],
    canon: "agent-skill-advisor-layer/routing-policy.yaml via agent-run",
    cloud: {
      local_http: false,
      acp: false,
...
async function runDsh(task, cwd, timeoutMs) {
  ...
  return spawnCaptured(bins.dsh, ["--profile", profile, task], { cwd, timeoutMs });
}
