...
import {
  ...
  missingCursorAcpAdapterWarning,
...
test("DSH_GATEWAY_PROFILE defaults to headless and never infers web", () => {
  assert.equal(resolveGatewayProfile({}), DEFAULT_GATEWAY_PROFILE);
  assert.equal(resolveGatewayProfile({ DSH_GATEWAY_PROFILE: "  " }), DEFAULT_GATEWAY_PROFILE);
  assert.equal(resolveGatewayProfile({ DSH_GATEWAY_PROFILE: "custom" }), "custom");
  assert.equal(usage().includes("DSH_GATEWAY_PROFILE"), true);
});

test("doctor warns when default provider is cursor-acp but the gateway profile lacks the adapter", () => {
  const fakeHome = mkdtempSync(join(tmpdir(), "gw-warn-"));
  const dshHome = join(fakeHome, ".dsh");
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "dsh-profile-headless", dependencies: {} }),
  );
  const report = doctor({ PATH: "/nonexistent", HOME: fakeHome, DSH_HOME: dshHome }, fakeHome);
  assert.equal(report.ok, false);
  assert.equal(report.profile, "headless");
  assert.equal(report.warnings.length, 1);
  assert.match(report.warnings[0], /NO_ADAPTER/);
  assert.match(report.warnings[0], /cursor-acp/);
  assert.equal(report.warnings[0].includes("web"), true);
  const warning = missingCursorAcpAdapterWarning(dshHome, "headless");
  assert.match(warning, new RegExp(CURSOR_ACP_ADAPTER.replace("/", "\\/")));
});

test("doctor stays warning-free when the gateway profile already has the cursor-acp adapter", () => {
  const fakeHome = mkdtempSync(join(tmpdir(), "gw-ok-"));
  const dshHome = join(fakeHome, ".dsh");
  mkdirSync(join(dshHome, "profiles", "headless"), { recursive: true });
  writeFileSync(
    join(dshHome, "settings.yaml"),
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      dependencies: { [CURSOR_ACP_ADAPTER]: "file:/tmp/example.tgz" },
    }),
  );
  const report = doctor({ PATH: "/nonexistent", HOME: fakeHome, DSH_HOME: dshHome }, fakeHome);
